...
renderCUDA(
    ...
    bool get_flag,
    float* __restrict__ metric_per_gs,
    float* __restrict__ metricCount,
    float* __restrict__ gs_weight,
    const float* __restrict__ gt_image)
{
    ...
    for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
    {
        ...
        for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++) {
            ...
            if (con_o.w >= 0) {
				...
				// if(get_flag) {
				// 	atomicAdd(&(metric_per_gs[collected_id[j]]), metric_map[pix_id] * alpha * T);
    ...
    if (get_flag && inside) {
        ...
        for (int ch = 0; ch < CHANNELS; ch++) {
            actual_diff += fabsf(out_color[ch * H * W + pix_id] - gt_image[ch * H * W + pix_id]);
        }
            ...
            for (int j = 0; inside && j < min(BLOCK_SIZE, toDo); j++) {
                ...
                if (con_o.w >= 0) {
                    ...
                    for (int ch = 0; ch < CHANNELS; ch++) {
						...
						if (alpha < 0.9f) {
							out_color_wo_or_w_current_gs = accum_color[ch] + (out_color[ch * H * W + pix_id] - current_color - accum_color[ch]) / (1 - alpha);
						} else {
                        ...
                        current_diff += fabsf(out_color_wo_or_w_current_gs - gt_image[ch * H * W + pix_id]);
